...
python manage.py runserver              运行开发服务器测试，确保开发服务器下能正常打开网站。
```

## 安装uWSGI
```
sudo apt install libpython3.6 - dev       ubuntu下安装
pip install uwsgi                         通过pip虚拟环境安装
```
...
## 运行
```
sudo uwsgi - -http 0.0.0.0: 8000 - -wsgi - file test.py - -processes 4 - -threads 3    0.0.0.0可以省略
...
sudo kill - 9 2208 2209                  这时根据 PID 可以用下面的命令 kill 掉相关程序：
```
...
```
```
uwsgi - -ini uwsgi.ini                   uwsgi启动
uwsgi - -stop uwsgi.pid                  uwsgi 停止
```

## 3. 安装nginx,在ubuntu下安装。
```
...
server {
    ...
    location / {
        # 必须和uwsgi.ini中socket一样,配置了upstream可以将uwsgi_pass配置为：http:// +
        # upstream名称，即“http://ray”.
